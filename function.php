...
<?php
	function add_page_content($page_data){
		if($page_data['page_name'] != NULL){
			...
			$content = htmlspecialchars($page_data['page_content'],ENT_QUOTES);
			$slug = str_replace(' ','-', strtolower($page_data['page_name']));
		$insert="insert into page values(NULL,'$page_data[page_name]','$content','".date('d-m-Y')."','$slug')";
		...
		$select="select * from page";
			$contents=mysql_query($select);
				while($con=mysql_fetch_array($contents)){
					...
					echo htmlspecialchars_decode($con['page_content'],ENT_QUOTES)."<br/>";
					echo $con['date']."<br/>";
			}
	}
	function signup($val){
		...
		$checkEmailQuery = "SELECT email from user where email='$val[email]'";
		$executeCheckEmailQuery = mysql_query($checkEmailQuery);
		$rows = mysql_num_rows($executeCheckEmailQuery);
		if( $rows > 0 )
		{
			//$_SESSION['msg'] = '<div class="alert alert-danger" role="alert">Email Already Registered</div>'; 
			echo "<script>alert('Your email is already registered. Please Login')</script>";
			
			echo "<script>window.location='user-login.php'</script>";
		} 
		else 
		{
			$ins = "insert into user(email,twitter_id,password,type,member_since,validation_key,status) values('$val[email]','$val[twitter]','$val[password]','$val[type]','$membersince','$activation_key',0)";
			$query = mysql_query($ins) or die(mysql_error());
			$id =  mysql_insert_id();
			$ins1 = "insert into usermeta values('" .$id. "','user-detail','".json_encode( $val )."','$id')";
	...
	function transport($val){
		...
		if( $rows > 0 ){
			...
			$ins = "insert into user(email,password,type,member_since,validation_key,status) values('$val[email]','$val[password]','$val[type]','$membersince','$activation_key_trans',0)";
			$query = mysql_query($ins) or die(mysql_error());
			$ins1 = "insert into usermeta values('" . mysql_insert_id() . "','user-detail','".json_encode( $val )."','".mysql_insert_id()."')";
			$query = mysql_query($ins1) or die(mysql_error());
			if($query){
				//$_SESSION['msg'] = '<div class="alert alert-success" role="alert">Registration Completed , Please check your Email to get it activated.</div>';
							$email = $val['email'];
	...
	<table class="table table-striped table-bordered bootstrap-datatable datatable">
		...
		<?php while($d = mysql_fetch_array($query)){ ?>
			<tr>
                ...
                <td><?php echo $d['member_since']; ?></td>				
				<td><?php echo $value->type; ?></td>
                <td><?php echo $value->mobile; ?></td>
                <td><?php if ($d['status']==1) { echo "Active"; } else { echo "Inactive";} ?></td>	
				
				<td class="center">
					<a class="btn btn-danger" href="user.php?delete=<?php echo $d['id']; ?>"><i class="halflings-icon white trash"></i> </a>
				</td>
			</tr>
		<?php	} ?>
	...
	function post_trip($val){
		...
		$a = date('d M Y');
		$id = 0;
		$ins = "insert into post_journey(descripation,froms,tos,passenger,date,help,vehicle,facilities,user_id,created,price,journey_status,cancel_status) values('$val[descripation]','$val[from]','$val[to]','$val[passenger]','$val[date]','$val[help]','$vec','$fac','$user_id','$a','$val[price]','0','0')";
	...
	<?php while($d = mysql_fetch_array($query)){ ?>
			<tr>
				...
				<td><?php echo htmlspecialchars_decode($d['page_content'],ENT_QUOTES); ?></td>
	...
	function user_journey(){
		...
		while($d = mysql_fetch_array($query)){
			...
			if($d['journey_status'] == 1 ){
				$journey_current_status = "<a href='javascript:void(0)' class='btn btn-primary approve disabled' data-btn='$count'>Completed</a>";
			} else {
	...
	function chat_send_message($to,$from,$msg,$stat=0)
	{
		...
		if(mysql_num_rows($query) < 1)
		{
			...
			$unique = $today . $rand;
			$chat_group = "INSERT into chat_group (group_id,sender,reciever) values('".$unique."','$from','$to')";
		...
		if($stat == 1)
		{ $status = $stat; } else { $status = 0; }
		
		$msg=mysql_real_escape_string($msg);
		
		$msg_send = "INSERT into chat (sender,message,status,date,time,group_id) values ('$from','$msg','$status',CURDATE(),CURTIME(),'$_SESSION[group_id]')";
	...
	function last_interaction($id){
		...
		while($value = mysql_fetch_array($execute))
		{
			...
			if($value['sender'] == $id)
			{
				...
				if($num_of_pending_msgs == 0)
				{ unset($num_of_pending_msgs); 
	...
	function chats($id){
		$send = "select * from chat where group_id='$id' order by id";
		$send_query = mysql_query($send);
		$count = 0;
			while($d = mysql_fetch_array($send_query))
			{
				$profileimg = get_chat_profile_image($d['sender']);
				?>
				<div class="media msg"> 
					<a class="pull-left" href="javascript:void(0);"> 
					<?php if($profileimg == NULL){ ?>
						<img src="images/avatar.jpg" style="width: 40px; height: 40px;">
					<?php } else { ?>
						<img src="Profile_Picture/<?php echo $profileimg;?>" style="width: 40px; height: 40px;"> 
					<?php } ?>
					</a>
					<div class="media-body"> 
						<small class="time"><i class="fa fa-clock-o"></i> <?php echo $d['time'];?> </small>
						<h5 class="media-heading"><?php echo select_user_name($d['sender']);?></h5>
						<small class="col-lg-10"><?php echo $d['message'];?></small> 
					</div>
				</div>
			<?php 					
				$count++;
		...
		//echo "<div class='message-wrap col-sm-12'>";
